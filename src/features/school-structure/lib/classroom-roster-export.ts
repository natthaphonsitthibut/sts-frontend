...
function printPdf(title: string, columns: RosterExportColumn[], rows: string[][]): void {
  ...
  popup.document.write(`<!doctype html><html lang="th"><head><meta charset="utf-8"><title>${escapeXml(title)}</title><style>@page{size:A4 landscape;margin:14mm}body{font-family:sans-serif;color:rgb(15 23 42)}h1{font-size:20px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:${escapeXml(primaryColor || "currentColor")};color:white}th,td{border:1px solid rgb(203 213 225);padding:8px;text-align:left}</style></head><body><h1>${escapeXml(title)}</h1><table><thead><tr>${header}</tr></thead><tbody>${body}</tbody></table><script>window.onload=()=>{window.print();window.close()}<${"/"}script></body></html>`);
