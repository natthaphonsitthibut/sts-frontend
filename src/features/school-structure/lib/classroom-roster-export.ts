...
import sarabunBoldUrl from "../../../assets/fonts/thsarabunpsk-bold.woff2?url";
import sarabunRegularUrl from "../../../assets/fonts/thsarabunpsk-regular.woff2?url";
...
function printPdf(
  ...
  const header = columns
    .map((column) => `<th>${escapeXml(column.label)}</th>`)
    .join("");
  const body = rows
    .map(
      (row) =>
