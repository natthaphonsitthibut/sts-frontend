...
export function NotificationBell() {
  ...
  // Keep the bell on the same query as the dialog so the unread total cannot
    ...
    <div className="relative z-10">
      ...
      >
        <Bell
          ...
          className={cn(
            "size-5",
            unseenCount > 0 && !open && "animate-notification-bell motion-reduce:animate-none",
          )}
          strokeWidth={2.1}
        />
        {unreadCount > 0 ? (
      ...
      <NotificationCenterDialog onOpenChange={setOpen} open={open} />
    </div>
  );
}
