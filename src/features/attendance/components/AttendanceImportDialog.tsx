...
export function AttendanceImportDialog({
  ...
  const canApply = !disabled && !isParsing && (result?.matches.length ?? 0) > 0;
            ...
            <DialogBody className="space-y-5">
              ...
              <section className="space-y-2">
                ...
                <p className="text-xs text-slate-500">
                  ลิงก์ต้องแชร์แบบ “ผู้ที่มีลิงก์” และรองรับ Google Sheets,
                  Google Drive, OneDrive และ SharePoint
                </p>
              </section>
          ...
          <>
            <DialogBody className="space-y-3">
              ...
              <p className="text-xs text-slate-500">
                ...
                และมีรายชื่อห้องนี้ครบทุกคน โดยช่องสถานะเป็นตัวเลือกให้กด
                ไม่ต้องพิมพ์เอง
              </p>
            </DialogBody>
  ...
  if (result.missingColumns.length > 0) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          ไม่พบคอลัมน์ {result.missingColumns.join(", ")} — ดาวน์โหลดแบบฟอร์มจาก
          “ตัวอย่างไฟล์” แล้วกรอกในไฟล์นั้น
        </AlertDescription>
      </Alert>
    );
  }
