import { downloadXlsx, type SpreadsheetColumn } from "../../../lib/spreadsheet-file";
...
export const ATTENDANCE_IMPORT_COLUMNS: readonly AttendanceImportColumnDefinition[] = [
  { key: "order", label: "ลำดับ", aliases: ["ที่", "no", "order"], required: false },
  {
    key: "studentNumber",
    label: "รหัสประจำตัว",
    aliases: ["รหัสนักเรียน", "เลขประจำตัว", "เลขประจำตัวนักเรียน", "studentnumber", "studentid"],
    required: false,
  },
  {
    key: "name",
...
function resolveColumnIndexes(
  ...
  for (const column of ATTENDANCE_IMPORT_COLUMNS) {
    ...
    const index = normalizedHeaders.findIndex(
      (header, position) =>
        !taken.has(position) && header.length > 0 && candidates.includes(header),
    );
...
export function resolveAttendanceImport(
  ...
  if (indexes.studentNumber < 0 && indexes.name < 0) {
    missingColumns.push(`${ATTENDANCE_IMPORT_COLUMNS[1].label} หรือ ${ATTENDANCE_IMPORT_COLUMNS[2].label}`);
  }
  ...
  for (const student of roster) {
    ...
    if (number) {
      byStudentNumber.set(number, [...(byStudentNumber.get(number) ?? []), student]);
    }
  ...
  sheet.rows.forEach((row, position) => {
    ...
    const rowLabel = [studentNumber, name].filter(Boolean).join(" · ") || `แถวที่ ${rowNumber}`;

    const addIssue = (reason: AttendanceImportIssueReason, detail: string): void => {
